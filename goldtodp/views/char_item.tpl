            ...
            {if $this->data_model->GetCountAccount($realm->getId())}
                <center>
                    <div style="padding: 8px 35px 8px 14px; margin-bottom: 20px; text-shadow: 0 1px 0 rgba(255, 255, 255, 0.5); background-color: #593C58; border: 1px solid #FF006A; -webkit-border-radius: 4px; -moz-border-radius: 4px; border-radius: 4px;">
                        <div style="text-align: right;"><span style="color: blue;"><span
                                        style="font-family: tahoma,geneva,sans-serif;"><span
                                            style="font-size: 12px;"></span></span></span></div>
                        <div style="text-align: right;"></div>

                        <div>  {lang("Free", "goldtodp")}

                            {if $Server_fee != 0}
                                <img src="{$url}application/images/icons/coins_add.png" align="absmiddle" height="12"
                                     width="12"/>
                    ...
                    <div class="select_character">
                        <div class="character store_item">
                            ...
                            <section class="character_buttons">
                                {if   $this->realms->getRealm($realm->getId())->getCharacters()->isOnline($character.guid)}
                                    <a href="javascript:void(0)"
                                       class="nice_button"> {lang("ShortCharacterOnline", "goldtodp")} </a>
                                {else}
                                    <a href="javascript:void(0)" class="nice_button"
                                       onClick="CharacterClass.selectCharacter(this, {$realm->getId()},'{$character.guid}','{$character.name}','{$this->data_model->Calculate_money({$this->data_model->getmoneychar($realm->getId(),{$character.guid})})}')">
                            ...
                            <img class="item_icon" width="45" height="45"
                                 src="{$url}application/images/avatars/{$this->data_model->Getavator($character)}.gif"
                                 data-tip=" Go Profile <br/><img src='{$url}application/images/stats/{$character.class}.gif' align='absbottom'/> {$character.name}(Lv{$character.level})  "
                                 align="absmiddle">

                            <a class="character_name" href="{$url}character/{$realm->getId()}/{$character.guid}"
                               data-tip=" Go Profile <br/><img src='{$url}application/images/stats/{$character.class}.gif' align='absbottom'/> {$character.name}(Lv{$character.level}) ">{$character.name}</a>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <div id="demo">
                                    ...
                                    <img src="{$url}application/modules/goldtodp/img/money-gold.gif" align="absmiddle"
                                         height="12" width="12"/>
                            ...
                            <tr>
                                <section class="tool_buttons">
                                    <center>
                                        ...
                                        <a href="javascript:void(0)" class="nice_button"
                                           onClick="CharacterClass.conversion(this)"> {lang("convert", "goldtodp")}</a>
                                    </center>
                                </section>
                            </tr>
                            <br/>
